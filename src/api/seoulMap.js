...
/**
 * 보이는소화기 전체 목록을 가져옵니다.
 * @param {(items: object[], total: number) => void} onProgress - 페이지 도착마다 호출되는 콜백
 */
export async function fetchExtinguishers(onProgress) {
  ...
  const totalCount = parseInt(h.TOTAL_COUNT ?? h.total_count ?? 0, 10)
  const firstItems = (Array.isArray(first.body) ? first.body : []).map(mapThemeItem)

  // 1페이지 데이터 즉시 화면에 반영
  if (onProgress) onProgress(firstItems, totalCount)
  ...
  if (totalPages > 1) {
    ...
    for (const r of results) {
      if (r.status === 'fulfilled') {
        const items = Array.isArray(r.value.body) ? r.value.body.map(mapThemeItem) : []
        allItems = [...allItems, ...items]
        // 페이지마다 누적 데이터 콜백
        if (onProgress) onProgress(allItems, totalCount)
      }
    }
  }
  ...
  // 캐시 저장
  try {
    sessionStorage.setItem(CACHE_KEY, JSON.stringify({ data: allItems, ts: Date.now() }))
  } catch { /* 무시 */ }
